...
gulp.task('publish', ['compile', 'dist'], (done) => {
  ...
  let args = [npm, 'publish', '--with-rmc-tools'];
  if (beta) {
...
gulp.task('guard', (done) => {
  ...
  if (npmArgs) {
    for (let arg = npmArgs.shift(); arg; arg = npmArgs.shift()) {
      if (/^pu(b(l(i(sh?)?)?)?)?$/.test(arg) && npmArgs.indexOf('--with-rmc-tools') < 0) {
        reportError();
        done(1);
        return;
      }
...
gulp.task('react-native-init', (done) => {
  ...
  if (!bundleName) {
    done(`rmc-tools now supports react-native@0.39~0.44, please check your version.`);
    return;
  }
